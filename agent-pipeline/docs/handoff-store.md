...
Discoveries are durable but do not automatically become backlog. Each names its `assertion`: `observed` for a fact seen directly, or `absence` for a claim that something is missing. An absence carries `proof { cmd, exit: 0, observation }`; a statement that no test, log or control exists is never accepted on intuition. Missing `lands` means `parking`; `criterion`, `regression` and `delivery_blocker` are the only classifications that block closure and each names the fact that blocks it. `findings.mjs` renders the triage inbox from `discoveries_declared`. A closure writes that list — including an explicit empty list — in the same `store-update` request as its state transition, so a rejected discovery write cannot be followed by a successful, silent closure. An active spec's issue list changes only through an operator-approved `scope_change`; after `ready_for_pr`, that approval must name `kind: "delivery_blocker"`, and after `merged` the list is immutable.
