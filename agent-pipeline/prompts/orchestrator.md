...
**Write `discoveries_declared` on the source issue as you persist the handoff.** On closure, it must be in the *same* `store-update` request as `pipeline_state`, even when it is `[]`; a write error can then never be followed by a separately successful closure. `findings.mjs` provides the triage inbox as a virtual view over those durable entries, so there is no second store to synchronise. Closure is refused only for `criterion`, `regression` and `delivery_blocker`; parked and framework findings travel with a successful closure.
