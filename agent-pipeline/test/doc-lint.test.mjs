...
/**
 * Prepares a sandbox carrying sources and a `doc_lint` block.
 *
 * @param files - pairs of relative path and content
 * @param settings - extra `doc_lint` fields, for example `skip`
...
function withSources(files, settings = {}) {
  ...
  config.doc_lint = { roots: ["src"], ...settings };
...
/**
 * Sources whose only undocumented export lives under `src/generated`.
 *
 * @returns the files for {@link withSources}
 */
function withGeneratedOffender() {
  return {
    "src/a.ts": "/**\n * Answers the meaning of things.\n */\nexport function meaning() {}\n",
    "src/generated/b.ts": "export function silent() {}\n",
  };
}

describe("doc-lint: the skip configuration reaches the walk", () => {
  test("honours the skip pattern given as a regex string", () => {
    sandbox = withSources(withGeneratedOffender(), { skip: "generated" });
  ...
  test("accepts a skip list of glob patterns, the shape the frontend bundles ship", () => {
    ...
    // reported as carrying no contract.
    sandbox = withSources(withGeneratedOffender(), { skip: ["**/generated/**"] });
    const result = run(sandbox, "doc-lint.mjs");
  ...
  test("refuses a skip that is neither a regex string nor a list of patterns", () => {
    ...
    const result = run(sandbox, "doc-lint.mjs");
    assert.notEqual(result.status, 0, "a skip read as nothing is a read of everything, green for the wrong reason");
    assert.match(result.output, /doc_lint\.skip/);
  });
});
