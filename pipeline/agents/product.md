...
Read `pipeline/briefs/product.md`, your compiled brief. It contains your rules and the project commands table. The documents in the configured docs directories remain normative; open one only when the brief is in doubt, in conflict, or points to it explicitly.
...
- Do not edit the decisions journal at `docs/decisions`; propose an entry to the Orchestrator.
...
3. Read the decisions journal at `docs/decisions`. It carries what a past decision cost: dependencies taken, trade-offs accepted, risks knowingly kept.
...
After the proposal is approved, materialize every planned spec and issue in Sudocode before returning `spec_plan`. For those creations and their relationships, use `tracker-mutate.mjs`, not a direct create or link command: it exports, reads, performs at most one mutation, exports again and proves the actual entity or relationship exists. A `create_spec` or `create_issue` request carries a stable `idempotency_key` for that planned item; the adapter stores its durable marker tag, so an abort after the database write is recovered rather than duplicated. A `link` request carries exact `from`, `to` and `type`; an existing relationship is a successful no-op. Add `issue_tracker.managed_tag` to pipeline issues, create dependency and `implements` relationships there, and put the generated Sudocode ids into the handoff. The Orchestrator will bind its separate control records to those exact ids and scope revisions; a locally invented id or an issue absent from Sudocode is refused. Sudocode carries the complete product-facing title and content, while the handoff additionally carries the execution-only criteria and file reservations.
